...
vec3 StarLayer(vec2 uv) {
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      ...
      float val = max(max(base.r, base.g), max(base.b, 0.0));
...
export function GalaxyField({
  ...
  useEffect(() => {
    ...
    try { const probe = document.createElement("canvas"); webgl = probe.getContext("webgl2") ?? probe.getContext("webgl"); }
    catch { return; }
    if (!webgl) return;
  ...
  return <div ref={ctnDom} className={`galaxy-container ${className ?? ""}`} aria-hidden="true" style={style} />;
}
