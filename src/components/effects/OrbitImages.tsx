...
function OrbitItem({ item, index, totalItems, path, itemSize, rotation, progress, fill }: OrbitItemProps) {
  ...
  const zIndex = useTransform(progress, (p: number) => {
    const offset = (((p + itemOffset) % 100) + 100) % 100;
    return Math.sin((offset / 100) * Math.PI * 2) >= 0 ? 20 : 2;
  });

  const opacity = useTransform(progress, (p: number) => {
    const offset = (((p + itemOffset) % 100) + 100) % 100;
    const sinVal = Math.sin((offset / 100) * Math.PI * 2);
    return sinVal < 0 ? 0.9 : 1;
  });
...
export interface OrbitImagesProps {
  ...
  altPrefix?: string;
  shape?: 'ellipse' | 'circle' | 'square' | 'rectangle' | 'triangle' | 'star' | 'heart' | 'infinity' | 'wave' | 'custom' | string;
  customPath?: string;
  baseWidth?: number;
  radiusX?: number;
  radiusY?: number;
  ...
  duration?: number;
  itemSize?: number;
  direction?: 'normal' | 'reverse' | string;
  fill?: boolean;
  width?: number | string;
